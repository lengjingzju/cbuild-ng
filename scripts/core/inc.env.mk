...
clang_gccdir   := $(shell LANG=en_US PATH=$(PATH) $(CROSS_COMPILE)gcc -print-search-dirs | grep "^libraries:" | cut -d '=' -f 2 | cut -d ':' -f 1)
clang_ldflags  += -L$(clang_gccdir) -Wl,-rpath-link=$(clang_gccdir) -B$(clang_gccdir)
ifeq ($(USING_CLANG_CXX_BUILD),y)
# Cross-compiling C++ requires providing C++ headers and libraries of LLVM.
# For example, cross compile aarch64 on Debian, the following packages need to be installed:
# sudo dpkg --add-architecture arm64
# sudo apt update && sudo apt install libc++-dev:arm64 libc++abi-dev:arm64
# Note: CMake failed to cross compile with clang because it did not use the set ldflags
#       during the try_compile phase. While IMake does not have this issue.
clang_incdir   ?= /usr/include/c++/v1
clang_libdir   ?= /usr/lib/$(shell echo $(CROSS_COMPILE) | sed 's/-$$//')
clang_cpflags  += -I$(clang_incdir)
clang_ldflags  += -L$(clang_libdir) -Wl,-rpath-link=$(clang_libdir) -stdlib=libc++
endif # USING_CLANG_CXX_BUILD
endif # CROSS_COMPILE
endif # clang
